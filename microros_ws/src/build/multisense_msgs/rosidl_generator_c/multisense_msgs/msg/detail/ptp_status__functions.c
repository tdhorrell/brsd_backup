  ...
  multisense_msgs__msg__PtpStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    multisense_msgs__msg__PtpStatus * data =
      (multisense_msgs__msg__PtpStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!multisense_msgs__msg__PtpStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          multisense_msgs__msg__PtpStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
