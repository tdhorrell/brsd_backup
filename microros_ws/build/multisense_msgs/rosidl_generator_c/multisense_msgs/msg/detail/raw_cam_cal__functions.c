  ...
  multisense_msgs__msg__RawCamCal__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    multisense_msgs__msg__RawCamCal * data =
      (multisense_msgs__msg__RawCamCal *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!multisense_msgs__msg__RawCamCal__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          multisense_msgs__msg__RawCamCal__fini(&data[i]);
        }
        free(data);
        return false;
      }
