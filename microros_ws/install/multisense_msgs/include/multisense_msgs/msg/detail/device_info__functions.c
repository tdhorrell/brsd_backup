  ...
  multisense_msgs__msg__DeviceInfo__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    multisense_msgs__msg__DeviceInfo * data =
      (multisense_msgs__msg__DeviceInfo *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!multisense_msgs__msg__DeviceInfo__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          multisense_msgs__msg__DeviceInfo__fini(&data[i]);
        }
        free(data);
        return false;
      }
