  ...
  multisense_msgs__msg__RawLidarCal__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    multisense_msgs__msg__RawLidarCal * data =
      (multisense_msgs__msg__RawLidarCal *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!multisense_msgs__msg__RawLidarCal__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          multisense_msgs__msg__RawLidarCal__fini(&data[i]);
        }
        free(data);
        return false;
      }
